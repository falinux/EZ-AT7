//------------------------------------------------------------------------------
// 화일명 : zmodem.c
// 설  명 : ezload의 zmodem 구현 루틴이다. 
// 
// 작성자 : 유영창 에프에이리눅스주식회사 frog@falinux.com
// 작성일 : 2001년 10월 8일
//------------------------------------------------------------------------------
...
void SerialOutStr( char *str, int size )
{
	int	lp ;

	for (lp=0;lp<size;lp++) console_putc(str[lp]);
}
...
static int zgethdr(char *hdr, int eflag, size_t *Rxpos)
{
	...
	max_garbage = Zrwindow + zmodem_baud;
	Rxframeind = Rxtype = 0;
...
int serial_trygetchar(int port)
{
	...
	if ( console_read_ok() ) ch = console_getc();
